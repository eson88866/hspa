...
class RouteScenario(BasicScenario):
	...
	def _create_test_criteria(self):
		...
		if DATA_COLLECTION:
			collision_criterion = CollisionTest(self.ego_vehicles[0],route=route , terminate_on_failure=True)
			red_light_criterion = RunningRedLightTest(self.ego_vehicles[0], terminate_on_failure=True)
		else:
			collision_criterion = CollisionTest(self.ego_vehicles[0],route=route, terminate_on_failure=False)
			red_light_criterion = RunningRedLightTest(self.ego_vehicles[0], terminate_on_failure=False)
		route_criterion = InRouteTest(self.ego_vehicles[0],
									  route=route,
									  offroad_max=30,#30
									  terminate_on_failure=True) #True
		...
		blocked_criterion = ActorSpeedAboveThresholdTest(self.ego_vehicles[0],
														 speed_threshold=0.1,
														 below_threshold_max_time=60.0, #180.0
														 terminate_on_failure=True,
														 name="AgentBlockedTest")
		
		adastest = ADASTest(self.ego_vehicles[0],route=route)###
