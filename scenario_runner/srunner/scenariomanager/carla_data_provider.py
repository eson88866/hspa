...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def create_blueprint(model, rolename='scenario', color=None, actor_category="car"):
        ...
        no_bike = True###We don’t want to use bicycles when testing ADAS
        # Set the model
        try:
            if no_bike:
                filtered_blueprints = [blueprint for blueprint in CarlaDataProvider._blueprint_library.filter(model)if blueprint.id not in ['vehicle.bh.crossbike', 'vehicle.diamondback.century','vehicle.gazelle.omafiets']]
                # print("###filtered_blueprints:",filtered_blueprints)
                blueprint = CarlaDataProvider._rng.choice(filtered_blueprints)
