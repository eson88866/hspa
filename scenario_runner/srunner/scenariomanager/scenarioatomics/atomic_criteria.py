...
class CollisionTest(Criterion):#revised
    ...
    def __init__(self, actor, route, other_actor=None, other_actor_type=None,
        ...
        ###
        self._route = route
        self._waypoints, self._command = zip(*self._route)
        self._route_length = len(self._route)
        self._map = CarlaDataProvider.get_map()
        self._current_index = 0
        self.front_collision = 0
        self.turn_left_collision = 0
        self.turn_right_collision = 0
        self.lanechange_collision = 0
        self.junction_collision = 0
        # VOID = 0
        # LEFT = 1
        # RIGHT = 2
        # STRAIGHT = 3
    ...
    def _count_collisions(weak_self, event):     # pylint: disable=too-many-return-statements
        ...
        for index in range(self._current_index,
                            min(self._current_index + self.WINDOWS_SIZE + 1, self._route_length-1)):
            ref_waypoint = self._waypoints[index]
            distance = math.sqrt(((actor_location.x - ref_waypoint.x) ** 2) + ((actor_location.y - ref_waypoint.y) ** 2))
            if distance <= shortest_distance:
                closest_index = index
                shortest_distance = distance
        self._current_index = closest_index
        cd = self._command[closest_index]
        _command = cd.value

        current_driving_wp = self._map.get_waypoint(actor_location, lane_type=carla.LaneType.Driving, project_to_road=True)
        _is_junction = current_driving_wp.is_junction
        ...
        # waypoint.is_junction
        if _command == 1:
            collision_type = "_turn_left"
            if actor_type == TrafficEventType.COLLISION_VEHICLE:
                self.turn_left_collision += 1
        if _command == 2:
            collision_type = "_turn_right"
            if actor_type == TrafficEventType.COLLISION_VEHICLE:
                self.turn_right_collision += 1
        if _command in [3, 4]:
            if _is_junction:
                collision_type = "_junction"
                if actor_type == TrafficEventType.COLLISION_VEHICLE:
                    self.junction_collision +=1
            else:
                collision_type = "_forward"
                if actor_type == TrafficEventType.COLLISION_VEHICLE:
                    self.front_collision += 1
        if _command in [5, 6]:
            collision_type = "_lanechange"
            if actor_type == TrafficEventType.COLLISION_VEHICLE:
                self.lanechange_collision +=1
        ...
        collision_event.set_dict({
            ...
            'turn_left_collision':self.turn_left_collision,
            'turn_right_collision':self.turn_right_collision,
            'front_collision':self.front_collision,
            'lanechange_collision':self.lanechange_collision,
            'junction_collision':self.junction_collision,###
            })
        collision_event.set_message(
            "Agent collided against object with type={} and id={} at (x={}, y={}, z={}) while {}".format(
...
class ADASTest(Criterion):
    WINDOWS_SIZE = 5  # Amount of additional waypoints checked
    
    def __init__(self, actor, route, name="ADASTest"):
        """
        Constructor
        """
        super(ADASTest, self).__init__(name, actor, 0, None, False)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))

        self._actor = actor
        self._map = CarlaDataProvider.get_map()
        self._previous_steer = 0.0
        self._previous_throt = 0.0
        self._previous_brake = 0.0
        self._num_steer_05 = 0
        self._num_throt_05 = 0
        self._num_brake_05 = 0
        self._num_steps = 0
        self._total_dev = 0.0
        self._num_dev_1 = 0
    ...
    def update(self):
        """
        Update the throttle and brake differences and calculate the averages.
        """
        ...
        # Check if termination on failure is enabled and the test status is failure
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            # Set the new status to failure
            new_status = py_trees.common.Status.FAILURE

        # Get the location of the actor
        location = CarlaDataProvider.get_location(self._actor)
        # If the location is not available, return the new status
        if location is None:
            return new_status
        
        ### Deviation Test
        # Get the current position
        current_position = Point(location.x, location.y)
        # Initialize the shortest distance and closest index
        shortest_distance = float('inf')
        closest_index = -1

        # Loop through the windows size waypoints
        for index in range(self._current_index,
                            min(self._current_index + self.WINDOWS_SIZE + 1, self._route_length-1)):
            # Calculate the distance between the current position and the waypoints
            wp1 = Point(self._waypoints[index].x, self._waypoints[index].y)
            wp2 = Point(self._waypoints[index+1].x, self._waypoints[index+1].y)
            distance = self.point_to_segment_distance(current_position, wp1, wp2)
            # If the distance is less than the shortest distance, update the closest index and shortest distance
            if distance <= shortest_distance:
                closest_index = index
                shortest_distance = distance

        # If closest index is -1 or shortest distance is infinity, return the new status
        if closest_index == -1 or shortest_distance == float('inf'):
            return new_status

        # Update the total deviation and check if the shortest distance is greater than 1
        self._total_dev += shortest_distance
        ...
        # Get the current control of the actor
        control = self._actor.get_control()
        # Get the current steer, throttle, and brake values
        current_steer = control.steer
        current_throt = control.throttle
        current_brake = control.brake

        # Calculate the differences between the current and previous values
        steer_diff = abs(current_steer - self._previous_steer)
        throt_diff = abs(current_throt - self._previous_throt)
        brake_diff = abs(current_brake - self._previous_brake)

        # Increment the number of steps
        self._num_steps += 1

        # Check if the steer, throttle, and brake differences are greater than 0.5
        if steer_diff > 0.5:
            self._num_steer_05 +=1
        if throt_diff > 0.5:
            self._num_throt_05 +=1
        if brake_diff > 0.5:
            self._num_brake_05 +=1
        
        # Update the previous values
        self._previous_steer = current_steer
        self._previous_throt = current_throt
        self._previous_brake = current_brake

        # Get the current speed of the actor
        current_speed = CarlaDataProvider.get_velocity(self._actor)
        # Update the max speed if the current speed is greater
        if current_speed > self._max_speed:
            self._max_speed = current_speed

        # Add the current speed to the total speed
        self._total_speed += current_speed
    ...
    def terminate(self, new_status):
        """
        Calculate the average throttle and brake differences and generate a traffic event.
        """
        avg_dev = self._total_dev / self._num_steps if self._num_steps > 0 else 0.0
        avg_speed = self._total_speed / self._num_steps if self._num_steps > 0 else 0.0

        # Create a traffic event for ADASTest
        adas_event = TrafficEvent(event_type=TrafficEventType.ADAS_TEST)
        adas_event.set_dict({
            'sec_steer>0.5':self._num_steer_05/20,
            'sec_throt>0.5':self._num_throt_05/20,
            'sec_brake>0.5':self._num_brake_05/20,
            'avg_dev':avg_dev,
            'sec_dev>1m':self._num_dev_1/20,
            'max_speed': self._max_speed*3.6,
            'avg_speed': avg_speed*3.6,
        }) #1 sec = 20 step =2 Hz

        self.list_traffic_events.append(adas_event)
        super(ADASTest, self).terminate(new_status)
    ...
    def point_to_segment_distance(self, p, v, w):
        ...
        Args:
            p (Point): The point.
            v (Point): The start point of the line segment.
            w (Point): The end point of the line segment.

        Returns:
            float: The distance between the point and the line segment.
        """
        # Define a named tuple to represent a point
        Point = namedtuple('Point', ['x', 'y'])

        # Calculate the squared length of the line segment
        l2 = (v.x - w.x) ** 2 + (v.y - w.y) ** 2

        # If the line segment is a point, return the distance between the point and the line segment
        if l2 == 0.0:
            return math.sqrt((p.x - v.x) ** 2 + (p.y - v.y) ** 2)

        # Calculate the parameter t that corresponds to the projection of the point onto the line segment
        t = ((p.x - v.x) * (w.x - v.x) + (p.y - v.y) * (w.y - v.y)) / l2

        # Constrain t to the range [0, 1]
        t = max(0, min(1, t))

        # Calculate the projection of the point onto the line segment
        projection = Point(v.x + t * (w.x - v.x), v.y + t * (w.y - v.y))

        # Calculate and return the distance between the point and the line segment
